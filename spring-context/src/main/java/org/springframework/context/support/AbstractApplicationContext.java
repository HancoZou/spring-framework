...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	 ...
	 *
	 * 这里初始化时，默认创建的是一个PathMatchingResourcePatternResolver
	 *
	 */
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 注册拦截Bean创建的Bean处理器（这里只是注册，真正的调用在getBean()的时候）
				registerBeanPostProcessors(beanFactory);

				// Initialize message source for this context.
				// 为上下文初始化Message源，即各种语言的消息体，国际化处理
				initMessageSource();
	...
	protected void prepareRefresh() {
		...
		// 在实际的使用中，可以通过重写initPropertySource()方法，在方法中定义必须检查的属性值，在validateRequiredProperties()中进行验证
	 ...
	 * @see #getBeanFactory()
	 *
	 * 将会复用BeanFactory中的配置文件读取解析及其他功能，
	 ...
	 *
	 * Qualifier与Autowired应该是大家非常熟悉的注解，那么这两个注解正是在这一步骤中增加的支持
	 *
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 默认可以使用 #{bean.***} 的形式来获取相关属性值
		beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));
		// 为beanFactory增加一个默认的propertyEditor，主要是对bean的属性等设置管理的一个工具
		// 这里仅仅是注册了一个ResourceEditorRegistrar实例，真正注册在AbstractBeanFactory.initBeanWrapper中
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks.
		// 添加beanPostProcessor
		// 真正的逻辑还是在ApplicationContextAwareProcessor中
	 ...
	 * 这种开放式的设计在Spring中随处可见，例如在本例中就提供了一个空的函数实现postProcess BeanFactory来方便程序员在业务上做进一步扩展
	 *
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// ConversionService 也是Spring提供的一个转换类型
	...
	protected void finishRefresh() {
		...
		// 完成ApplicationContext初始化的时候，要通过Spring中的事件发布机制来发出Context RefreshedEvent事件，以保证对应的监听器可以做进一步的逻辑处理
